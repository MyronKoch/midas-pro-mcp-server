...
export function registerControlTools(server) {
    // ── connect ──────────────────────────────────────────────────────────────
    server.tool("connect", "Connect to a Midas Pro Series console on the network. You must connect before using get_value or set_value. Requires the console's IP address. Default OSC port is 10023.", {
    ...
    // ── disconnect ───────────────────────────────────────────────────────────
    server.tool("disconnect", "Disconnect from the Midas console.", {}, async () => {
        await midasClient.disconnect();
        return {
            content: [
                { type: "text", text: "Disconnected from Midas console." },
            ],
        };
    });
    // ── connection_status ────────────────────────────────────────────────────
    server.tool("connection_status", "Check whether the MCP server is currently connected to a Midas console, and if so, which one.", {}, async () => {
    ...
    // ── get_value ────────────────────────────────────────────────────────────
    server.tool("get_value", "Read the current value of a parameter from the connected Midas console. Sends an OSC query and waits for the response. Works for all endpoint types including meters.", {
    ...
    // ── set_value ────────────────────────────────────────────────────────────
    server.tool("set_value", "Set a parameter value on the connected Midas console. Sends an OSC message with the appropriate argument type. Float values are clamped to 0-1 range. DANGEROUS commands (reboot, reset) are blocked and flagged.", {
        group: z.string().describe("Control group name"),
        endpoint: z.string().describe("Endpoint/parameter name"),
        value: z
            .union([z.number(), z.string()])
            .describe("Value to set. Float 0-1 for faders/rotaries, 0 or 1 for switches, string for labels."),
        index: z
            .number()
            .int()
            .min(0)

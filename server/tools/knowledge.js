import { z } from "zod";
import { listGroups, listEndpoints, getEndpointInfo, buildOscPath, searchEndpoints, getStats, } from "../endpoints.js";
/**
 * Register all Phase 1 knowledge-base tools on the MCP server.
 */
export function registerKnowledgeTools(server) {
    // ── list_groups ──────────────────────────────────────────────────────────
    server.tool("list_groups", "List all Midas Pro Series control groups (e.g. VirtualMicInputs, VirtualMasters, VirtualGEQPool) with endpoint counts and message type breakdowns. Start here to explore what the console can do.", {}, async () => {
        ...
        const lines = [
            ...
            ``,
            `Message types:`,
            ...Object.entries(stats.byMessageType).map(([type, count]) => `  ${type}: ${count}`),
            ``,
            `Control Groups:`,
            `${"─".repeat(70)}`,
        ];
    ...
    // ── list_endpoints ───────────────────────────────────────────────────────
    server.tool("list_endpoints", "List all endpoints in a specific control group with their descriptions, types, and argument info. Use list_groups first to see available groups.", {
        ...
        for (const { endpoint, spec } of endpoints) {
            ...
            lines.push(`  ${endpoint}${readOnly}${multi}`, `    Type: ${typeShort} | Arg: ${spec.argumentType ?? "none"} | ${spec.description}`, ``);
        }
        return {
            content: [{ type: "text", text: lines.join("\n") }],
        };
    ...
    // ── search_endpoints ─────────────────────────────────────────────────────
    server.tool("search_endpoints", "Search across all 4,596 endpoints by keyword. Matches endpoint names, descriptions, group names, and message types. Use this to find the right command for what you want to control (e.g. 'fader', 'mute', 'eq frequency', 'label').", {
        ...
        for (const r of capped) {
            ...
            lines.push(`  ${r.oscPath}${multi}${readOnly}`, `    Group: ${r.group} | Arg: ${r.spec.argumentType ?? "none"}`, `    ${r.spec.description}`, ``);
        }
        return {
            content: [{ type: "text", text: lines.join("\n") }],
        };
    ...
    // ── get_endpoint_info ────────────────────────────────────────────────────
    server.tool("get_endpoint_info", "Get complete details for a specific endpoint including its OSC path, message type, argument type, whether it supports multiple instances, and its description.", {
        ...
        if (spec.isAbsolute !== undefined) {
            lines.push(`Absolute:      ${spec.isAbsolute ? "Yes (sets value directly, not toggle)" : "No (toggle behavior)"}`);
        }
        // Add usage hints based on message type
        lines.push("", "Usage:");
        switch (spec.type) {
            case "enPPCFaderMessage":
                lines.push("  Send a float between 0.0 and 1.0 (exclusive).", "  Example: 0.75 = ~75% fader position");
                break;
            case "enPPCRotaryMessage":
                lines.push("  Send a float between 0.0 and 1.0 (inclusive).", "  The float maps to the control's range (e.g. 20Hz-20kHz for frequency).", "  Value mapping depends on the specific parameter.");
                break;
            case "enPPCSwitchMessage":
                lines.push(spec.isAbsolute
            ...
            case "enPPCMeterMessage":
                lines.push("  READ-ONLY. Send with no arguments to get current meter value.", "  Returns a float between 0.0 and 1.0.");
                break;
        }
        if (spec.multiPath) {
            lines.push("", "Indexing:", "  Append /<index> to the OSC path (0-based).", "  Example: .../0 = first instance (input 1, channel 1, etc.)");
        }
        return {
            content: [{ type: "text", text: lines.join("\n") }],
        };
    ...
    // ── build_osc_command ────────────────────────────────────────────────────
    server.tool("build_osc_command", "Construct the complete OSC path string for a command, ready to send. Validates the group, endpoint, and index. Returns the exact path you would send over UDP.", {
        group: z.string().describe("Control group name"),
